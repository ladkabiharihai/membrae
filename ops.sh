...
# the corpora are already built: data/window2_train.bin -> /mnt/kv_cache/pragnosia_data/window2_train.bin
# (330GB / 176.6B tokens, CoT-enriched). Build the SPIN-DOMINANT run with carrier="spin_dominant" in
# pragnosia.json (vs carrier="none" for the transformer baseline) -- see HANDOVER.md.
cd /opt/code/membrae
...
status)
  ...
  $PY -c "import json;c=json.load(open('pragnosia.json'));print('-- arch: layers='+str(c['layers'])+' mlp_mult='+str(c['mlp_mult'])+' carrier='+c.get('carrier','single'))" 2>/dev/null
...
resume)
  ...
  # --lr 0 restores the derived peak lr from the sidecar (find_lr is unreliable on trained weights). This is the
  # NO-GROW refine path; for a born-small SPIN-DOMINANT run that should self-grow, drop --no-grow.
