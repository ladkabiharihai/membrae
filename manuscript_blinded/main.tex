...
\begin{abstract}
We study a transformer/state-space hybrid in which a rotational ``spin'' recurrence, a diagonal-complex linear-recurrent unit run as a parallel associative scan, is the \emph{core token-mixer}, with attention only a periodic helper. The work has a sharp, honest arc. We first scaled the hybrid the wrong way: a single spin carrier placed after a transformer stack. Its parameter share collapses $5.76\% \to 1.90\% \to 0.37\%$ from 21M to 1.4B, and at 1.4B a causal probe shows the carrier contributes only 0.028\% of the loss, vestigial as built. A parameter-matched ablation then shows the intended design. With spin as the mixer, validation perplexity is 219 vs 279 for an attention-only baseline at matched parameters, a quality win when spin is the core \emph{at that scale}. A control that swaps the carrier for a real, no-phase recurrence in the same placement also beats attention ($3.5\times$), so at the tested budget the effect is about placement, not phase. Separately, we establish the design's core property at scale as causal attribution. In a 284M spin-dominant model (validation perplexity 24.54 on a 176.6B-token corpus), ablating the spin carrier sends perplexity $25 \to 7716$ ($306\times$), while ablating attention costs only $3.4\times$: the carrier is the load-bearing core, the exact inverse of the 1.4B drift. We then measured that advantage \emph{across scale}, with matched parameters, corpus, tokens and learning rate. At 100M a parameter-matched pair over three paired seeds gives spin 28.80 vs attention 29.06: spin leads in 3/3 seeds but by only 0.88\%, not significant at $n=3$ ($p\approx0.14$), and the per-seed gap shrinks. At 284M the crux, a matched attention-only transformer at the same budget, is a tie (21.01 vs 20.92). The 21\% advantage at 37M therefore does \emph{not} survive scale: spin-dominant is competitive with a matched transformer at 100M--284M and does not collapse (the inverse of the 1.4B drift), but it is not superior. On this substrate runs a proposed self-calibrating controller (mechanisms described; validation preliminary): surprise-modulated continual learning with generative replay, a persistent in-weights episodic memory, an introspection layer, and function-preserving self-growth on probe-confirmed saturation, with no hand-set calibration thresholds. We report the limits plainly: the spin-dominant advantage is small-scale only (parity by 100M--284M, three paired seeds at 100M), the grown models are undertrained, and the honesty signal does not yet separate knowledge from confabulation at 284M.
\end{abstract}
