...
\author{\normalsize\textit{Author name(s) withheld for double-blind review}}
\date{July 2026}
...
\begin{abstract}
We study a transformer/state-space hybrid in which a diagonal-complex ``spin'' recurrence, run as a parallel associative scan, is the \emph{core token-mixer} and attention only a periodic helper. Scaled the wrong way, as a single carrier placed after a transformer stack, the recurrence becomes vestigial: its parameter share collapses to $0.37\%$ by 1.4B and a causal probe attributes only $0.028\%$ of the loss to it. Making the carrier the core inverts this. At 37M the spin-dominant design beats a parameter-matched attention-only baseline by about $21\%$ (219 vs 279 perplexity). We then measured that advantage \emph{across scale}, with matched parameters, tokens and learning rate, and it does not survive: at 100M three paired seeds give 28.80 vs 29.06 ($+0.88\%$, not significant), and at 284M a matched attention-only transformer ties it (21.01 vs 20.92). What does hold is causal. Ablating the carrier costs two orders of magnitude more perplexity than ablating attention, at every scale and in all three seeds ($313\times$ vs $4.6\times$ at 284M): the recurrence is genuinely the load-bearing computation, the exact inverse of the 1.4B drift. Spin-dominant is therefore \emph{competitive} with a matched transformer at scale, not superior. We add a preliminary self-calibrating controller, and release the system, every measured number, and every limitation.
\end{abstract}
...
Param-matched abl.\ (37M) & 219 vs 279 & spin-dominant by 21\%, small scale \\
Matched pair, 100M (3 seeds) & 28.80 vs 29.06 & spin $+0.9\%$, n.s.: parity \\
Crux: matched tf., 284M & 20.92 vs 21.01 & tie: advantage gone \\
...
\textbf{A fast--slow reading.} This split has a brain-inspired form. Cortical processing, and recent neuromorphic designs, pair a fast local pathway with a slow, low-dimensional state that carries context over long timescales and modulates the fast dynamics~\cite{sun2026dualmemory}. Our architecture is the same decomposition in a language model: the attention blocks are the fast, local pathway, and the diagonal-complex spin carrier is the slow, persistent-context pathway. Read this way, our central question, ``which pathway is load-bearing?'', is the natural test of the fast--slow hypothesis at language-model scale, and our answer, that the slow carrier carries the computation once it is the core, is a statement about where context should live. We also tested the stronger form of that hypothesis, letting the slow carrier \emph{modulate} the fast attention pathway inside a block (a compact low-dimensional slow state gating attention, after~\cite{sun2026dualmemory}). Trained from scratch at 284M against an identical control differing only in the coupling (100K steps, matched budget and learning rate), it was 7.2\% \emph{worse} (26.65 vs 24.87 validation perplexity). We report this as an honest negative: at this scale the two pathways are better left parallel than coupled.
...
\begin{align}
b_t &= g \odot (U_{\mathrm{re}}\, y_t + i \, U_{\mathrm{im}}\, y_t), \\
h_t &= \lambda \odot h_{t-1} + b_t, \quad \lambda_j = e^{-\exp(\nu_j)}\, e^{\,i\theta_j}, \\
\mathrm{out}_t &= C[\,\mathrm{Re}\, h_t ; \mathrm{Im}\, h_t\,], \quad x_t \mathrel{+}= \sigma(\gamma)\, \mathrm{out}_t. \nonumber
\end{align}
...
\begin{figure*}[t]\centering\includegraphics[width=0.92\textwidth]{figs/arch.pdf}
...
\subsection{The intended design is spin-dominant, and wins at matched parameters (small scale)}
...
We trained the intended design at real scale for the first time. A 284M model ($d=768$, 20 layers, \code{mlp\_mult}$=9$, carrier as mixer in 15 of 20 blocks, attention every 4th) trained to step 418{,}000 on a 176.6B-token chain-of-thought corpus, reaching best validation perplexity 24.54 (about 24 tokens/param; we report this as a description of the run, not a compute-optimality claim, since the corpus is deliberately reasoning-heavy). \textbf{Two 284M runs, and why both appear here.} This original run predates a learning-rate bug we found later: the adaptive controller could only \emph{cut} the rate, so early-training noise could ratchet it to its floor and pin it there (the same fault later held the matched transformer baseline at perplexity 174.8 until fixed). A corrected re-run of the identical 284M configuration reaches \textbf{20.92}, and it is that corrected pair that we compare in the crux (Section~\ref{sec:limitations}). The bug moved the perplexity, not the conclusion: ablating the carrier on the \emph{corrected} 284M costs $313\times$ versus $4.6\times$ for attention (gap $68\times$), reproducing the original run's gap, and the property replicates over three independently-trained seeds (Section~\ref{sec:mechanism}). Perplexities below the corrected figure are from the original growth chain and are labelled as such. In the spin-dominant configuration the carrier is intra-block with no cross-segment state to transplant, so the equivalent causal measure is direct ablation.
...
For external comparability we evaluate the 284M model zero-shot on standard public sets. Because the model uses a custom 16K-token vocabulary, we report word-normalized perplexity and log-likelihood multiple-choice accuracy, both tokenizer-independent (Table~\ref{tab:bench}). The scores are honest and modest, above chance on most tasks and at chance on the hardest (ARC-Challenge), the expected profile of a small model undertrained for its size. We additionally report two harder axes on the 1B: MMLU (broad knowledge, 4-choice) at $27.9\%$ (1{,}000 questions, just above the $25\%$ chance floor) and GSM8K (grade-school multi-step math) at $1.3\%$ (150 problems, greedy generation with exact numeric match). Both confirm the profile, near-chance broad knowledge and near-zero multi-step arithmetic for a small model undertrained on a reasoning-heavy custom corpus; we report them rather than omit them.
...
\begin{table*}[t]\centering\small\caption{Zero-shot accuracy (\%) vs open models of similar size, one harness, 1{,}000-example subsamples. Comparable to GPT-2 (124M) and Cerebras-256M, behind the larger GPT-2-medium; ARC-Challenge sits at chance for all four models.}
\label{tab:open}
...
\begin{figure*}[t]\centering\includegraphics[width=0.92\textwidth]{figs/progression.pdf}
...
\begin{table*}[t]\centering\small\caption{Our 1B (spin-dominant) vs a roughly token-matched transformer (illustrative, non-controlled). On the multi-hop task (ARC-Challenge) both sit at chance; the transformer's lead is concentrated on general-English fluency, where our custom corpus and tokenizer are confounds.}
\label{tab:cerebras}
\begin{tabular}{@{}l r r r r r r@{}}
\toprule
Model & Params & PIQA & ARC-Easy & ARC-Challenge & HellaSwag & LAMBADA \\
