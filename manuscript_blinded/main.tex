...
\begin{table*}[t]\centering\small\caption{Summary of the central results. These are two claims, kept distinct: a small-scale \emph{quality} win (spin-dominant beats a matched transformer) and a large-scale \emph{causal-dominance} property (the carrier is load-bearing). The 284M rows show the carrier is used, not that the design wins at scale.}
\label{tab:summary}
