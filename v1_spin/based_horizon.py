"""mh-Based long-needle HORIZON (self-contained, no slow import side-effects). Based only (fast cumsum).
Does the Taylor-sharpness carrier recall 1 fact across L=256/512/1024 filler tokens? = our long-context task."""
...
def needle_batch(B, L, K=32, V=32, Fr=64):
    T = 2 + L + 1; x = torch.zeros(B, T, dtype=torch.long); y = torch.full((B, T), -100, dtype=torch.long)
    for b in range(B):
        k = random.randint(1, K); v = random.randint(K + 1, K + V); fill = [random.randint(K + V + 1, K + V + Fr) for _ in range(L)]
        x[b] = torch.tensor([k, v] + fill + [k]); y[b, -1] = v
    return x.to(DEV), y.to(DEV)

def run(L, steps=3000, lr=2e-3, B=48):
    vocab = 32 + 32 + 64 + 1; torch.manual_seed(0)
    M.CARRIERS["c"] = lambda d: MHBased(d, 4, 12)
...
print("mh-Based long-needle HORIZON (Based only):")
for L in [256, 512, 1024]:
    print(f"  L={L:>4}: {run(L)*100:5.1f}%", flush=True)
