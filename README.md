...
| `grow.py` | **Neurogenesis on demand** — function-preserving growth (add neurons/layers) so the brain expands its own capacity when saturated, with zero forgetting at the moment of growth. `Brain.grow()` wires it in. |
