...
if __name__=="__main__":
    ...
    if os.environ.get("RESUME")=="1" and sn:
        ...
        _sd=torch.load(latest,map_location=DEV); _sd={k.replace("_orig_mod.",""):v for k,v in _sd.items()}  # tolerate compile-prefixed snapshots
        m.load_state_dict(_sd); print(f"RESUMED {latest} E={Es} step{start}",flush=True)
